...
fn main() {
  ...
  tauri::Builder::default()
    .setup(|_app| {
      database::api::init();  // Initialize the database.
      Ok(())
    })
    //.manage(state)
    .plugin(tauri_plugin_oauth::init())
    .invoke_handler(tauri::generate_handler![new_transaction, get_transactions, fix_transaction, remove_transaction, new_account, get_accounts, fix_account, remove_account, login, register, fix_user, link::auth::authenticate, link::auth::authorize])
