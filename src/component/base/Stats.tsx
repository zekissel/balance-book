...
export default function Stats({ transactions, accounts, signalRefresh, setRange }: StatsProps) {
	...
	const [timeRange, setTimeRange] = useState(
		sessionStorage.getItem('stats.timeRange')
			? Number(sessionStorage.getItem('stats.timeRange'))
			: 2,
	)
	const [rangeMultiplier, setRangeMultiplier] = useState(
		sessionStorage.getItem('stats.timeMulti')
			? Number(sessionStorage.getItem('stats.timeMulti'))
			: 7,
	)
	...
	const endDate = useMemo(() => {
		if (filters.endDate !== null) return filters.endDate
			...
			return addDays(
				transactions.reduce((max, p) => (p.date > max ? p.date : max), new Date()),
				1,
			)
	...
	const startDate = useMemo(() => {
		if (filters.startDate !== null) return filters.startDate
			...
			return addDays(
				transactions.reduce((min, p) => (p.date < min ? p.date : min), new Date()),
				-1,
			)
		} else {
			const ret = new Date(
				new Date(
					endDate.getTime() - (timeRange * rangeMultiplier - 1) * 24 * 60 * 60 * 1000,
				).toDateString(),
			)
			return ret
		}
	...
	const timeFrameTransactions = useMemo(() => {
		...
		return filteredTransactions.filter(
			(t) =>
				t.date.getTime() - startDate.getTime() >= 0 && endDate.getTime() - t.date.getTime() >= 0,
		)
		...
		<div className="page-root">
			<menu className="dynamic-menu">
				<div className="dynamic-menu-main">
					<button
						id={!anyDateFilters() && rangeMultiplier > 0 ? 'dynamic-menu-current' : undefined}
					>
						<input type="text" value={timeRange} onChange={updateRangeDays} />
					</button>
					<button
						id={!anyDateFilters() && rangeMultiplier === 1 ? 'dynamic-menu-current' : undefined}
						onClick={() => updateMultiplier(1)}
					>
						<img src="/cal-day.svg" /> Days
					</button>
					<button
						id={!anyDateFilters() && rangeMultiplier === 7 ? 'dynamic-menu-current' : undefined}
						onClick={() => updateMultiplier(7)}
					>
						<img src="/cal-week.svg" /> Weeks
					</button>
					<button
						id={!anyDateFilters() && rangeMultiplier === 30 ? 'dynamic-menu-current' : undefined}
						onClick={() => updateMultiplier(30)}
					>
						<img src="/calendar.svg" /> Months
					</button>
					<button
						id={!anyDateFilters() && rangeMultiplier === 365 ? 'dynamic-menu-current' : undefined}
						onClick={() => updateMultiplier(365)}
					>
						<img src="/cal-year.svg" /> Years
					</button>
				</div>
