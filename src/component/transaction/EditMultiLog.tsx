...
interface EditMultiLogProps { editLogs: string[], logs: Transaction[], accounts: Account[], cancel: () => void }
export default function EditMultiLog ({ editLogs, logs, accounts, cancel }: EditMultiLogProps) {
  ...
  const handleSubmitEdit = async () => {
    for (const id of editLogs) {
      const log = { 
        ...
        amount: editAmount !== '0' ? (Math.round((Number(editAmount) + Number.EPSILON) * 100) * (isIncome ? 1 : -1)) : logs.find(l => l.id === id)?.amount,
  ...
  const displayAmount = useMemo(() => `${(Math.abs(Number(editAmount)))}${editAmount.charAt(editAmount.length - 1) === '.' ? '.' : ''}${(editAmount.charAt(editAmount.length - 2) === '.' && editAmount.charAt(editAmount.length - 1) === '0') ? '.0' : ''}`, [editAmount]);
  ...
  return (
    <menu className='edit-multi-log'>
      <span>
        <button onClick={cancel}><img src='/x.svg'/></button>
      </span>
      <span>
        Edit {editLogs.length} {isIncome === true ? 'Income(s)' : isIncome === false ? 'Expense(s)' : 'Transactions'}
      </span>

      <label htmlFor='edit-multi-co'>Company: </label>
      <input type='text' id='edit-multi-co' placeholder='Company' onChange={e => setEditCompany(e.target.value)} />

      <label htmlFor='edit-multi-am'>Amount: </label>
      <input type='text' id='edit-multi-am' placeholder={'Amount'} value={!logConflict ? displayAmount : 'Amount disabled'} onChange={updateAmount} disabled={logConflict} />
      ...
      { isIncome === undefined && <input type='text' id='edit-multi-ca' placeholder='Category disabled' disabled={logConflict} /> }

      <label htmlFor='edit-multi-da'>Date: </label>
      <input type='date' id='edit-multi-da' value={editDate?.toISOString().substring(0, 10)} onChange={(e) => {
          setEditDate(new Date(new Date(e.target.value).toUTCString().split(' ').slice(0, 4).join(' ')));
          }} />

      <label htmlFor='edit-multi-de'>Description: </label>
      <input type='text' id='edit-multi-de' placeholder='Description' onChange={e => setEditDesc(e.target.value)} />
