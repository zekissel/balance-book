...
export default function HeatMap({ transactions, startDate, endDate }: GraphProps) {
	...
	const rangeStart = useMemo(() => {
		...
		if (string.split('-')[1].length === 1) {
			string = string.slice(0, 5) + '0' + string.slice(5);
		}
	...
	const max = Math.round(
		(timeFrameTotals.reduce((max, t) => (t.total > max ? t.total : max), 0) * 1.05) / 100,
	);
	const min = Math.round(
		(timeFrameTotals.reduce((min, t) => (t.total < min ? t.total : min), 0) * 1.05) / 100,
	);
	...
	const option = {
		...
		series: {
			...
			data: timeFrameTotals.map((t) => [
				echarts.time.format(t.date, '{yyyy}-{MM}-{dd}', false),
				t.total / 100,
			]),
		},
	};
