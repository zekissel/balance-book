...
export default function TotalGraph ({ transactions, range, endDate }: GraphProps) {
  ...
  const option = {
    ...
    series: [
      {
        data: timeFrameTotals.map(t => new Object({value: t.total, itemStyle: { color: t.total > 0 ? '#739d88' : '#f6d6aa' }, label: {show:t.total !== 0,position:t.total > 0 ?'top':'bottom',formatter: '${c}'} })),
        type: 'bar',
      }
    ],
    title: {
